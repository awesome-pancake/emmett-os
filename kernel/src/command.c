...
int parse_command(int argc, char **argv) {
	...
	for(int i=0; i<argc; i++){
		...
		if(!parse_state && kstrncmp("initheap", token, 9) == 0){
			parse_state = true;
			prints("\n");

                        // Heap debug code
			struct heap *heap_ptr = init_heap((void*)0x2010, 1);
                        int *pointer = (int*)kmalloc(heap_ptr, 48);
                        *pointer = 0xFFFFFFFF;

                        char *array = (char*)kmalloc(heap_ptr, 48);
                        for(int i = 0; i < 48; i++){
                                array[i] = 0x88;
                        }
